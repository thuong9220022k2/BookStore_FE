            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="tab-content" id="nav-tabContent">
                        <div v-if="state.orders.length > 0" class="tab-pane fade show active py-3" id="nav-all"
                            role="tabpanel" aria-labelledby="nav-all-tab">
                            <table v-for="(order, index) in state.orders" :key="order._id"
                                class="table mt-3 mb-5 shadow" cellspacing="0">
                                <!-- <thead>
                                    <tr>
                                        <th class="text-start"> {{ order.status === 0 ? "NGÀY ĐẶT HÀNG: " : order.status
                                            === 1 ? "ĐƯỢC DUYỆT VÀO NGÀY: " : order.status === 2 ? "BÀN GIAO CHO NHÀ VẬN
                                            CHUYỂN VÀO NGÀY: " : order.status === 3 ? "ĐƠN HÀNG ĐÃ BỊ HỦY VÀO NGÀY: " :
                                        ...
                                        <th :style="{ color: order.status === 0 ? 'grey' : order.status === 1 ? 'hotpink' : order.status === 2 ? 'orange' : order.status === 4 ? 'green' : 'red', }"
                                            class="text-end">
                                            {{ order.status === 0 ? "🕔 ĐANG CHỜ DUYỆT" : order.status === 1 ? "📦 ĐANG
                                            GÓI HÀNG" : order.status === 2 ? "🚚 ĐANG VẬN CHUYỂN" : order.status === 3 ?
                                            "❌ ĐÃ HỦY" :
                                ...
                                <thead>
                                    <tr>
                                        <th class="text-start">
                                            {{ order.status === 0 ? "NGÀY ĐẶT HÀNG: " :
                                                order.status === 1 ? "ĐƯỢC DUYỆT VÀO NGÀY: " :
                                                    order.status === 2 ? "BÀN GIAO CHO NHÀ VẬN CHUYỂN VÀO NGÀY: " :
                                                        order.status === 3 ? "ĐƠN HÀNG ĐÃ BỊ HỦY VÀO NGÀY: " :
                                                            "ĐƠN HÀNG ĐÃ ĐƯỢC GIAO VÀO NGÀY: "
                                            }} {{ order.created_at }}
                                        </th>
                                        <th></th>
                                        <th :style="{
                                            color: order.status === 0 ? 'grey' :
                                                order.status === 1 ? 'hotpink' :
                                                    order.status === 2 ? 'orange' :
                                                        order.status === 4 ? 'green' :
                                                            'red'
                                        }" class="text-end">
                                            {{ order.status === 0 ? "🕔 ĐANG CHỜ DUYỆT" :
                                                order.status === 1 ? "📦 ĐANG GÓI HÀNG" :
                                ...
                                <tbody>
                                    <tr v-for="(product, index) in order.products" :key="product.product_id">
                                        <td class="text-start row">
                                            <div class="col-3"><img :src="product.image" alt="" style="width: 100px" />
                                            </div>
                                    ...
                                    <tr class="bg-white " style="border-color:white">
                                        <td colspan="3" class="text-end pt-0 mt-0 pb-4">
                                            <router-link
                                                :to="{ name: 'customer.order.detail', params: { id: order._id } }">
                        ...
                        <div v-else class="tab-pane fade show active text-center py-3" id="nav-all" role="tabpanel"
                            aria-labelledby="nav-all-tab">
                            <img src="https://img.freepik.com/vector-premium/paquete-pila-d-icono-caja-carton-pila-vector_88813-2412.jpg"
                                class="img-fluid w-25 py-4" alt="">
                            <h5>Hiện chưa có đơn hàng nào được đặt!</h5>
                            <button class="btn btn-secondary mt-2">Tiếp tục mua sắm</button>
                        </div>
                        <div v-if="state.orders.length > 0" class="tab-pane fade py-3" id="nav-0" role="tabpanel"
                            ...
                            <div v-if="hasPendingOrders1">
                                <div v-for="(order, index) in state.orders" :key="order._id">
                                    <table class="table mt-3 mb-5 shadow" cellspacing="0" v-if="order.status === 0">
                                        <!-- <thead>
                                            <tr>
                                                <th class="text-start"> {{ order.status === 0 ? "NGÀY ĐẶT HÀNG: " :
                                                    order.status === 1 ? "ĐƯỢC DUYỆT VÀO NGÀY: " : order.status === 2 ?
                                                        "BÀN GIAO CHO NHÀ VẬN CHUYỂN VÀO NGÀY: " : order.status === 3 ? "ĐƠN
                                                    HÀNG ĐÃ BỊ HỦY VÀO NGÀY: " :
                                                "ĐƠN HÀNG ĐÃ ĐƯỢC GIAO VÀO NGÀY: " }} {{ order.created_at }}</th>
                                                <th></th>
                                                <th :style="{ color: order.status === 0 ? 'grey' : order.status === 1 ? 'hotpink' : order.status === 2 ? 'orange' : order.status === 4 ? 'green' : 'red', }"
                                                    class="text-end">
                                                    {{ order.status === 0 ? "🕔 ĐANG CHỜ DUYỆT" : order.status === 1 ?
                                                        "📦 ĐANG GÓI HÀNG" : order.status === 2 ? "🚚 ĐANG VẬN CHUYỂN" :
                                                            order.status === 3 ? "❌ ĐÃ HỦY" :
                                                                "💵 ĐƠN HÀNG ĐÃ GIAO THÀNH CÔNG" }}</th>
                                            </tr>
                                        </thead> -->
                                        ...
                                        <tbody>
                                            <tr v-for="(product, index) in order.products" :key="product.product_id">
                                                <td class="text-start row">
                                                    <div class="col-3"><img :src="product.image" alt=""
                                            ...
                                            <tr class="bg-white " style="border-color:white">
                                                <td colspan="3" class="text-end pt-0 mt-0 pb-4">
                                                    <router-link
                                                        :to="{ name: 'customer.order.detail', params: { id: order._id } }">
                                                        <button type="button" class="btn btn-secondary w-25">Chi
                                                            tiết</button>
                                                    </router-link>
                                                    <button v-if="order.status === 0 || order.status === 1"
                                                        type="button"
                                                        @click="updateCancelOrderStatus(order._id, { status: 3 })"
                                                        class="btn btn-danger ms-3 w-25">Hủy
                                                    </button>
                        ...
                        <div v-else class="tab-pane fade text-center py-3" id="nav-0" role="tabpanel"
                            aria-labelledby="nav-0-tab">
                            <img src="https://img.freepik.com/vector-premium/paquete-pila-d-icono-caja-carton-pila-vector_88813-2412.jpg"
                                class="img-fluid w-25 py-4" alt="">
                            <h5>Hiện chưa có đơn hàng nào đang chờ duyệt!</h5>
                        ...
                        <div v-if="state.orders.length > 0" class="tab-pane fade py-3" id="nav-1" role="tabpanel"
                        ...
                        <div v-else class="tab-pane fade text-center py-3" id="nav-1" role="tabpanel"
                            aria-labelledby="nav-1-tab">
                            <img src="https://img.freepik.com/vector-premium/paquete-pila-d-icono-caja-carton-pila-vector_88813-2412.jpg"
                                class="img-fluid w-25 py-4" alt="">
                            <h5>Hiện chưa có đơn hàng nào đang chờ vận chuyển!</h5>
                        ...
                        <div v-if="state.orders.length > 0" class="tab-pane fade py-3" id="nav-2" role="tabpanel"
                        ...
                        <div v-else class="tab-pane fade text-center py-3" id="nav-2" role="tabpanel"
                            aria-labelledby="nav-2-tab">
                            <img src="https://img.freepik.com/vector-premium/paquete-pila-d-icono-caja-carton-pila-vector_88813-2412.jpg"
                                class="img-fluid w-25 py-4" alt="">
                            <h5>Hiện chưa có đơn hàng nào đang chờ giao hàng!</h5>
                        ...
                        <div v-if="state.orders.length > 0" class="tab-pane fade py-3" id="nav-3" role="tabpanel"
                        ...
                        <div v-else class="tab-pane fade text-center py-3" id="nav-3" role="tabpanel"
                            aria-labelledby="nav-3-tab">
                            <img src="https://img.freepik.com/vector-premium/paquete-pila-d-icono-caja-carton-pila-vector_88813-2412.jpg"
                                class="img-fluid w-25 py-4" alt="">
                            <h5>Hiện chưa có đơn hàng nào bị hủy!</h5>
                        ...
                        <div v-if="state.orders.length > 0" class="tab-pane fade py-3" id="nav-4" role="tabpanel"
                        ...
                        <div v-else class="tab-pane fade text-center py-3" id="nav-4" role="tabpanel"
                            aria-labelledby="nav-4-tab">
                            <img src="https://img.freepik.com/vector-premium/paquete-pila-d-icono-caja-carton-pila-vector_88813-2412.jpg"
                                class="img-fluid w-25 py-4" alt="">
                            <h5>Hiện chưa có đơn hàng nào đã giao thành công!</h5>
